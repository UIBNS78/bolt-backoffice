...
export class DmService {
  ...
  protected readonly apiUrl = `${environment.apiURL}/delivery-men`;
  ...
  getAllAsOptions(): Observable<DeliveryMenOptionsResponse[]> {
    return this.http.get<{ deliveryMen: DeliveryMenOptionsResponse[] }>(`${this.apiUrl}/all-options`).pipe(
      map(data => {
  ...
  getDetails(id: number): Observable<DeliveryManDetails> {
    return this.http.get<{ deliveryMan: DeliveryManDetails }>(`${this.apiUrl}/details/${id}`).pipe(
      map(data => data.deliveryMan)
    );
  }
  
  create(data: FormData): Observable<void> {
    return this.http.post<{ deliveryManId: number; userId: number }>(`${this.apiUrl}`, data).pipe(
